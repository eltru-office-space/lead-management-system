...
def company_detail_view(company_id):
    ...
    # Column 2 - Status & Quality
    with col2:
        new_contact_status = st.selectbox("Contact Status", ["", "bounced", "replied", "interested", "not interested", "needs follow-up", "no response"])
        new_email_quality = st.selectbox("Email Quality", quality_options)
        new_phone_quality = st.selectbox("Phone Quality", quality_options)
        new_last_call_outcome = st.selectbox("Last Call Outcome", call_outcomes)

    # Column 3 - Scheduling & Ownership
    with col3:
        new_first_contacted_date = st.date_input("First Contacted Date")
        new_next_follow_up_date = st.date_input("Next Follow-Up Date")
        new_outreach_channels = st.multiselect("Outreach Channels", outreach_options)
        new_updated_by_name = st.selectbox("Updated By", ["", "Corlene", "Jane"])

        # --- New Contact Source dropdown ---
        contact_source_options = ["", "costar", "apollo", "google", "given by lead", "other"]
        new_contact_source = st.selectbox("Contact Source", contact_source_options)
        ...
        if submitted:
            new_payload = {
                ...
                "first_contacted_date": new_first_contacted_date.isoformat() if new_first_contacted_date else None,
                ...
                "notes": new_notes,
                "updated_by": new_updated_by_name if new_updated_by_name else None
            }
            new_response = supabase.table("contacts").insert(new_payload).execute()
            if hasattr(new_response, "error") and new_response.error:
                st.error(f"❌ Adding contact failed: {new_response.error.message}")
            else:
